...
print('Your image are on the process of renaming................')
...
for image_identifier in range(1,scanned_input_image_count+1):
    ...
    (thresh, thresh_image) = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)
    ...
    for i, contour_box in enumerate(table_boxes):
        ...
        img = dilated_image[h1+5:h2-5,w1+5:w2-5]
  
        resized_img = cv2.resize(img, (128, 128))

        cv2.imwrite(os.path.join(output_dir,character_image_name+'_'+str(time.time())+str(i)+'.jpeg'),resized_img)
